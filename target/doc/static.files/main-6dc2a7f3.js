...
function getVirtualKey(ev) {
  if ("key" in ev && typeof ev.key !== "undefined") return ev.key;
...
(function () {
  ...
  function loadScript(url, errorCallback) {
    ...
    script.src = url;
    if (errorCallback !== undefined) script.onerror = errorCallback;
    ...
    settingsMenu.querySelector("a").onclick = (event) => {
      ...
      setTimeout(() => {
        ...
        for (const theme of themes) {
          if (theme !== "") preLoadCss(getVar("root-path") + theme + ".css");
        }
      }, 0);
    };
  });
  window.searchState = {
    ...
    showResults: () => {
      ...
      if (btnLabel) btnLabel.innerHTML = "Exit";
    },
    ...
    hideResults: () => {
      ...
      const btnLabel = document.querySelector("#search-button a span.label");
      if (btnLabel) btnLabel.innerHTML = "Search";
    },
    getQueryStringParams: () => {
      const params = {};
      window.location.search.substring(1).split("&").map((s) => {
        const pair = s.split("=").map((x) => x.replace(/\+/g, " "));
        params[decodeURIComponent(pair[0])] = typeof pair[1] === "undefined"
          ? null
    ...
    setup: () => {
      ...
      function loadSearch() {
        if (!searchLoaded) {
          ...
          if (!window.StringdexOnload) window.StringdexOnload = [];
      ...
      if (browserSupportsHistoryApi()) {
        ...
        window.addEventListener("popstate", (e) => {
          ...
          if (params.search !== undefined && inputElement !== null) {
            ...
            e.preventDefault();
            window.searchState.showResults();
            if (params.search === "") window.searchState.focus();
          } else window.searchState.hideResults();
        });
      }
      window.onpageshow = () => {
        const inputElement = window.searchState.inputElement();
        ...
        if (qSearch !== undefined && inputElement !== null) {
          if (inputElement.value === "") inputElement.value = qSearch;
          window.searchState.showResults();
          if (qSearch === "") {
            loadSearch();
  ...
  function handleHashes(ev) {
    ...
    if (savedHash !== pageId) {
      savedHash = pageId;
      if (pageId !== "") expandSection(pageId);
    }
        ...
        onEachLazy(implElems, (implElem) => {
          ...
          return onEachLazy(
            ...
            (item) => {
              ...
              if (
                item.id === assocId || (numbered && numbered[1] === assocId)
              ) {
  ...
  function handleShortcut(ev) {
    ...
    if (ev.ctrlKey || ev.altKey || ev.metaKey || disableShortcuts) return;
    if (
  ...
  function addSidebarItems() {
    if (!window.SIDEBAR_ITEMS) return;
    ...
    function block(shortty, id, longty) {
      ...
      for (const name of filtered) {
        let path;
        if (shortty === "mod") path = `${modpath}${name}/index.html`;
        else path = `${modpath}${shortty}.${name}.html`;
        let current_page = document.location.href.toString();
        if (current_page.endsWith("/")) current_page += "index.html";
        const link = document.createElement("a");
        link.href = path;
        link.textContent = name;
        const li = document.createElement("li");
        if (link.href === current_page) li.classList.add("current");
        li.appendChild(link);
        ul.appendChild(li);
      }
  ...
  window.register_implementors = (imp) => {
    ...
    const traitName =
      document.querySelector(".main-heading h1 > .trait").textContent;
    ...
    for (const lib of libs) {
      if (lib === window.currentCrate || ignoreExternCrates.has(lib)) continue;
      ...
      struct_loop: for (const struct of structs) {
        ...
        onEachLazy(code.getElementsByTagName("a"), (elem) => {
          ...
          if (
            href && !href.startsWith("#") && !/^(?:[a-z+]+:)?\/\//.test(href)
  ...
  window.register_type_impls = (imp) => {
    if (!imp || !imp[window.currentCrate]) return;
    ...
    for (const impList of imp[window.currentCrate]) {
      ...
      if (types.indexOf(selfPath) === -1) continue;
      ...
      onEachLazy(template.content.querySelectorAll("[id]"), (el) => {
        let i = 0;
        if (idMap.has(el.id)) i = idMap.get(el.id);
        else if (document.getElementById(el.id)) {
          i = 1;
          while (document.getElementById(`${el.id}-${2 * i}`)) i = 2 * i;
          while (document.getElementById(`${el.id}-${i}`)) i += 1;
        }
        if (i !== 0) {
          const oldHref = `#${el.id}`;
          ...
          onEachLazy(template.content.querySelectorAll("a[href]"), (link) => {
            if (link.getAttribute("href") === oldHref) link.href = newHref;
          });
        }
        idMap.set(el.id, i + 1);
      });
      ...
      if (isTrait) {
        ...
        sidebarTraitList.append(li);
      } else {onEachLazy(templateAssocItems, (item) => {
          ...
          if (!block) {
            ...
            if (hasClass(item, "associatedtype")) associatedTypes = block;
  ...
  function collapseAllDocs(collapseImpls) {
    ...
    onEachLazy(document.getElementsByClassName("toggle"), (e) => {
      if (
        (collapseImpls || e.parentNode.id !== "implementations-list") ||
  ...
  function toggleAllDocs(ev) {
    ...
    if (hasClass(innerToggle, "will-expand")) expandAllDocs();
    else collapseAllDocs(ev !== undefined && ev.shiftKey);
  }
  (function () {
    const toggles = document.getElementById(toggleAllDocsId);
    if (toggles) toggles.onclick = toggleAllDocs;
    ...
    onEachLazy(document.getElementsByClassName("toggle"), (e) => {
      ...
      if (hideMethodDocs && hasClass(e, "method-toggle")) e.open = false;
    });
  })();
  ...
  if (mainElem) mainElem.addEventListener("click", hideSidebar);
  ...
  onEachLazy(
    ...
    (el) => {
      el.addEventListener("click", (e) => {
        if (!e.target.matches("summary, a, a *")) e.preventDefault();
      });
    },
  );
  function showTooltip(e) {
    ...
    if (!window.NOTABLE_TRAITS && notable_ty) {
      ...
      if (data) window.NOTABLE_TRAITS = JSON.parse(data.innerText);
    ...
    if (finalPos > 0) wrapper.style.left = finalPos + "px";
    else {wrapper.style.setProperty(
    ...
    wrapper.onpointerleave = (ev) => {
      if (
        ev.pointerType !== "mouse" || !(ev.relatedTarget instanceof HTMLElement)
      ) return;
  ...
  function setTooltipHoverTimeout(element, show) {
    ...
    if (!show && !window.CURRENT_TOOLTIP_ELEMENT) return;
    if (show && window.CURRENT_TOOLTIP_ELEMENT) return;
    if (
      window.CURRENT_TOOLTIP_ELEMENT &&
    ...
    element.TOOLTIP_HOVER_TIMEOUT = setTimeout(
      () => {
        if (show) showTooltip(element);
        else if (!element.TOOLTIP_FORCE_VISIBLE) hideTooltip(false);
      },
      show
  ...
  function hideTooltip(focus) {
    if (window.CURRENT_TOOLTIP_ELEMENT) {
      if (window.CURRENT_TOOLTIP_ELEMENT.TOOLTIP_BASE.TOOLTIP_FORCE_VISIBLE) {
        if (focus) window.CURRENT_TOOLTIP_ELEMENT.TOOLTIP_BASE.focus();
  ...
  onEachLazy(document.getElementsByClassName("tooltip"), (e) => {
    ...
    e.onpointerleave = (ev) => {
      if (ev.pointerType !== "mouse") return;
      if (
        !e.TOOLTIP_FORCE_VISIBLE && window.CURRENT_TOOLTIP_ELEMENT &&
  ...
  const sidebar_menu_toggle =
    document.getElementsByClassName("sidebar-menu-toggle")[0];
  if (sidebar_menu_toggle) {
    sidebar_menu_toggle.addEventListener("click", () => {
      const sidebar = document.getElementsByClassName("sidebar")[0];
      if (!hasClass(sidebar, "shown")) showSidebar();
      else hideSidebar();
    });
  }
  function helpBlurHandler(event) {
    ...
    if (!isInPopover) window.hidePopoverMenus();
  }
...
<a href="${drloChannel}/rustdoc/">the rustdoc book</a>.`;
    const shortcuts = [
      ...
      x[0].split(" ").map((
        y,
        index,
      ) => ((index & 1) === 0 ? "<kbd>" + y + "</kbd>" : " " + y + " ")).join(
        "",
      ) + "</dt><dd>" + x[1] + "</dd>"
    ).join("");
    const div_shortcuts = document.createElement("div");
    addClass(div_shortcuts, "shortcuts");
    div_shortcuts.innerHTML = "<h2>Keyboard Shortcuts</h2><dl>" + shortcuts +
      "</dl></div>";
    const infos = [
    ...
    if (!isHelpPage) container.className = "popover content";
    container.id = "help";
    ...
    } else {onEachLazy(document.getElementsByClassName("help-menu"), (menu) => {
  ...
  if (isHelpPage) buildHelpMenu();
  else {onEachLazy(document.querySelectorAll(".help-menu > a"), (helpLink) => {
      helpLink.addEventListener("click", (event) => {
        if (event.ctrlKey || event.altKey || event.metaKey) return;
        event.preventDefault();
        if (document.getElementById("help")) window.hidePopoverMenus();
        else showHelp();
      });
    });}
  addSidebarItems();
...
(function () {
  ...
  if (sidebarButton) {
    sidebarButton.addEventListener("click", (e) => {
      ...
      if (window.rustdocToggleSrcSidebar) window.rustdocToggleSrcSidebar();
      e.preventDefault();
    });
  }
  ...
  const showSidebar = function () {
    if (isSrcPage) window.rustdocShowSourceSidebar();
    else {
      removeClass(document.documentElement, "hide-sidebar");
      updateLocalStorage("hide-sidebar", "false");
    }
  ...
  const resize = function (e) {
    if (currentPointerId === null || currentPointerId !== e.pointerId) return;
    e.preventDefault();
    const pos = e.clientX - 3;
    if (pos < SIDEBAR_VANISH_THRESHOLD) hideSidebar();
      ...
      pendingSidebarResizingFrame = setTimeout(() => {
        if (
          currentPointerId === null || pendingSidebarResizingFrame === false
        ) return;
  ...
  window.addEventListener("resize", () => {
    if (window.innerWidth < RUSTDOC_MOBILE_BREAKPOINT) return;
    stopResize();
    if (
  ...
  const initResize = function (e) {
    if (
      currentPointerId !== null || e.altKey || e.ctrlKey || e.metaKey ||
      e.button !== 0
    ) return;
...
(function () {
  ...
  but.onclick = () => {
    ...
    const path = [item];
    if (module !== undefined) path.unshift(module);
  ...
  function getExampleWrap(event) {
    ...
    if (target instanceof HTMLElement) {
      ...
      while (elem !== null && !hasClass(elem, "example-wrap")) {
        if (
          elem === document.body || elem.tagName === "A" ||
          elem.tagName === "BUTTON" || hasClass(elem, "docblock")
        ) return null;
        elem = elem.parentElement;
      }
      return elem;
  ...
  function addCopyButton(event) {
    ...
    if (runButton !== null) parent.appendChild(runButton);
