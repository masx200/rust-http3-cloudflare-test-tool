...
const initSearch = async function (Stringdex, RoaringBitmap, hooks) {
  ...
  const editDistanceState = {
    ...
    calculate: function calculate(a, b, limit) {
      ...
      if (minDist > limit) return limit + 1;
  ...
  function getFilteredNextElem(query, parserState, elems, isInGenerics) {
    ...
    if (
      ...
      if (elems.length === 0) throw ["Expected type filter before ", ":"];
      else if (query.literalSearch) throw ["Cannot use quotes on type filter"];
  ...
  function getItemsBefore(query, parserState, elems, endChar) {
    ...
    while (parserState.pos < parserState.length) {
      ...
      } else if (isEndCharacter(c)) throw ["Unexpected ", c, " after ", extra];
      if (!foundStopChar) {
        let extra = [];
      ...
      if (posBefore === parserState.pos) parserState.pos += 1;
      foundStopChar = false;
    }
  ...
  function getNextElem(query, parserState, elems, isInGenerics) {
    ...
    const handleRefOrPtr = (chr, name) => {
      ...
      if (parserState.userQuery.slice(pos, pos + 3) === "mut") {
        ...
        chr === "*" && parserState.userQuery.slice(pos, pos + 5) === "const"
      ) {
        parserState.pos += 5;
        c = parserState.userQuery[parserState.pos];
      }
    ...
    if ("[(".indexOf(parserState.userQuery[parserState.pos]) !== -1) {
      ...
      if (
        name === "()" && !foundSeparator && generics.length === 1 &&
        typeFilter === null
      ) elems.push(generics[0]);
      else if (
        name === "()" && generics.length === 1 && generics[0].name === "->"
      ) {
        ...
        parserState.totalElems += 1;
        if (isInGenerics) parserState.genericsElems += 1;
      ...
      } else end = getIdentEndPosition(parserState);
      if (
        parserState.pos < parserState.length &&
        parserState.userQuery[parserState.pos] === "<"
      ) {
        if (start >= end) throw ["Found generics without a path"];
        parserState.pos += 1;
        ...
        if (start >= end) throw ["Found generics without a path"];
        if (parserState.isInBinding) throw ["Unexpected ", "(", " after ", "="];
        parserState.pos += 1;
        const typeFilter = parserState.typeFilter;
        parserState.typeFilter = null;
        getItemsBefore(query, parserState, generics, ")");
        ...
        if (isReturnArrow(parserState)) {
          ...
          skipWhitespace(parserState);
          getFilteredNextElem(query, parserState, generics, isInGenerics);
          generics[generics.length - 1].bindingName = makePrimitiveElement(
            "output",
          );
        } else {generics.push(
      ...
      if (isStringElem) skipWhitespace(parserState);
      if (start >= end && generics.length === 0) return;
      if (parserState.userQuery[parserState.pos] === "=") {
        if (parserState.isInBinding) {
  ...
  function createQueryElement(
    ...
    if (path.startsWith("::")) throw ["Paths cannot start with ", "::"];
    else if (quadcolon !== null) throw ["Unexpected ", quadcolon[0]];
    const pathSegments = path.split(/(?:::\s*)|(?:\s+(?:::\s*)?)/).map((x) =>
      x.toLowerCase()
    );
    if (
      ...
      } else throw ["Unexpected ", parserState.userQuery[parserState.pos]];
    }
    for (const [i, pathSegment] of pathSegments.entries()) {
      if (pathSegment === "!") {
        if (i !== 0) throw ["Never type ", "!", " is not associated item"];
    ...
    return {
      ...
      generics: generics.filter((gen) => {
        if (gen.bindingName !== null && gen.bindingName.name !== null) {
          if (gen.name !== null) gen.bindingName.generics.unshift(gen);
          bindings.set(
  ...
  function getStringElem(query, parserState, isInGenerics) {
    if (isInGenerics) throw ["Unexpected ", '"', " in generics"];
    ...
    } else if (start === end) throw ["Cannot have empty string element"];
    parserState.pos += 1;
  ...
  function getIdentEndPosition(parserState) {
    ...
    while (parserState.pos < parserState.length) {
      ...
      if (c === "!") {
        ...
        if (c === ":") {
          if (!isPathStart(parserState)) break;
          parserState.pos += 1;
        } else {while (parserState.pos + 1 < parserState.length) {
        ...
        c === "[" || c === "(" || isEndCharacter(c) ||
        isSpecialStartCharacter(c) || isSeparatorCharacter(c)
      ) break;
      ...
      } else throw ["Unexpected ", c, " (not a valid identifier)"];
      parserState.pos += 1;
      afterIdent = consumeIdent(parserState);
      end = parserState.pos;
    }
    if (macroExclamation !== -1) {
      if (parserState.typeFilter === null) parserState.typeFilter = "macro";
  ...
  class VlqHexDecoder {
    ...
    next() {
      ...
      if (this.backrefQueue.length > 16) this.backrefQueue.pop();
      return result;
    }
  }
  ...
  function itemTypeFromName(typename) {
    if (typename === null) return NO_TYPE_FILTER;
    const index = itemTypes.findIndex((i) => i === typename);
    if (index < 0) throw ["Unknown type filter ", typename];
    return index;
  }
  class DocSearch {
    ...
    getTypeNameIds() {
      if (this.typeNameIds) return this.typeNameIds;
    ...
    static parseQuery(userQuery) {
      ...
      function parseInput(query, parserState) {
        ...
        while (parserState.pos < parserState.length) {
          ...
          if (query.elems.length === before) parserState.pos += 1;
          foundStopChar = false;
        }
        if (parserState.typeFilter !== null) {
          throw [
      ...
      try {
        ...
        const checkTypeFilter = (elem) => {
          ...
          for (const generic of elem.generics) checkTypeFilter(generic);
        ...
        for (const elem of query.elems) checkTypeFilter(elem);
        for (const elem of query.returned) checkTypeFilter(elem);
      } catch (err) {
        query = newParsedQuery(userQuery);
        if (
          Array.isArray(err) && err.every((elem) => typeof elem === "string")
        ) query.error = err;
        else throw err;
        return query;
      }
    ...
    async getFunctionData(id) {
      ...
      const parser = new VlqHexDecoder(raw[0], async (functionSearchType) => {
        if (typeof functionSearchType === "number") return null;
        const INPUTS_DATA = 0;
        const OUTPUT_DATA = 1;
        let inputs_;
        let output_;
        ...
        } else {inputs_ = this.buildItemSearchTypeAll(
        ...
        if (functionSearchType.length > 1) {
          ...
          } else {output_ = this.buildItemSearchTypeAll(
              functionSearchType[OUTPUT_DATA],
            );}
        } else output_ = Promise.resolve(EMPTY_GENERICS_ARRAY);
        const where_clause_ = [];
    ...
    async getTypeData(id) {
      ...
      } else if (raw[0]) searchUnbox = true;
      return {
        searchUnbox,
    ...
    async getGenericInvertedIndex(id) {
      ...
      if (pb === undefined || pb === null || pb.length === 0) return [];
    ...
    async buildItemSearchType(type) {
      ...
      if (typeof type === "number") {
        ...
        if (type[BINDINGS_DATA] && type[BINDINGS_DATA].length > 0) {
          bindings = new Map(
            (await Promise.all(type[BINDINGS_DATA].map(async (binding) => {
        ...
        } else bindings = EMPTY_BINDINGS_MAP;
      }
      let result;
      if (id < 0) {
        result = {
        ...
        if (
          path === undefined || path === null || type === undefined ||
          type === null
        ) {
          return {
            id: null,
        ...
        result = {
          ...
          exactPath: path.exactModulePath === null
            ? path.modulePath
      ...
      if (cr) {
        if (
          ...
          cr.generics !== result.generics && cr.generics.every((x, i) =>
            result.generics[i] === x
          )
        ...
        if (
          ...
          if (ok) result.bindings = cr.bindings;
        }
        if (
          cr.ty === result.ty && cr.path === result.path &&
    ...
    async execQuery(parsedQuery, filterCrates, currentCrate) {
      ...
      const buildHrefAndPath = (item) => {
        ...
        if (type === "mod") {
          displayPath = path + "::";
          href = this.rootPath + path.replace(/::/g, "/") + "/" + name +
            "/index.html";
        } else if (type === "import") {
          ...
          type === "primitive" || type === "keyword" || type === "attribute"
        ) {
          displayPath = "";
          exactPath = "";
          href = this.rootPath + path.replace(/::/g, "/") + "/" + type + "." +
            name + ".html";
      ...
      function pathSplitter(path) {
        ...
        if (tmp.endsWith("<span>")) return tmp.slice(0, tmp.length - 6);
        return tmp;
      }
      const formatDisplayTypeSignature = async (
        ...
        if (typeInfo !== "elems" && typeInfo !== "returned") {
          ...
          if (typeInfo === "elems") fnInputs = highlighted;
          else fnOutput = highlighted;
        }
        if (!fnInputs) fnInputs = objType.inputs;
        if (!fnOutput) fnOutput = objType.output;
        ...
        const pushText = (fnType, result) => {
          if (!!(result.length % 2) === !!fnType.highlighted) result.push("");
        ...
        const writeSpecialPrimitive = async (fnType, result) => {
          if (
            ...
            const [ob, sb] = fnType.id === typeNameIds.typeNameIdOfArray ||
                fnType.id === typeNameIds.typeNameIdOfSlice
              ? ["[", "]"]
              : ["(", ")"];
        ...
        const writeFn = async (fnType, result) => {
          if (fnType.id !== null && fnType.id < 0) {
            if (fnParamNames[-1 - fnType.id] === "") {
              ...
              for (const nested of generics) await writeFn(nested, result);
              return;
            } else if (mgens) {
            ...
            if (fnType.ty === TY_PRIMITIVE) {
              ...
              fnType.name === "" && fnType.bindings.size === 0 &&
            ...
            if (fnType.bindings.size > 0) {
              await onEachBtwnAsync(
                await Promise.all(
                  [...fnType.bindings.entries()].map(async (
                    [key, values],
                  ) => [await this.getName(key), values]),
                ),
                async ([name, values]) => {
                  if (
                    values.length === 1 && values[0].id < 0 &&
                    `${fnType.name}::${name}` ===
                      fnParamNames[-1 - values[0].id]
                  ) {
                    for (const value of values) await writeFn(value, []);
                    return true;
                  }
                  if (!hasBindings) {
                    hasBindings = true;
      ...
      const transformResults = (results, typeInfo, duplicates) => {
        ...
        for (const result of results) {
          ...
          if (item.id !== -1) {
            ...
            if (duplicates.has(obj.fullPath)) continue;
            if (obj.item.ty === TY_IMPORT && duplicates.has(res[2])) continue;
            if (duplicates.has(res[2] + "|" + TY_IMPORT)) continue;
      ...
      const sortAndTransformResults = async function* (
        ...
        for (const result of results.values()) {
          if (!result) continue;
          const item = result.item;
          if (filterCrates !== null && item.crate !== filterCrates) continue;
          if (item) result_list.push(result);
          else continue;
        }
      ...
      function unifyFunctionTypes(
        ...
        if (
          ...
          for (const [i, fnType] of fnTypesIn.entries()) {
            ...
            if (
              fnType.id !== null && fnType.id < 0 && queryElem.id !== null &&
              queryElem.id < 0
            ) {
              if (
                mgens && mgens.has(queryElem.id) &&
        ...
        for (let i = flast; i >= 0; i -= 1) {
          ...
          if (fnType.id !== null && queryElem.id !== null && fnType.id < 0) {
            ...
            mgensScratch.set(queryElem.id, fnType.id);
          } else mgensScratch = mgens;
          fnTypes[i] = fnTypes[flast];
          fnTypes.length = flast;
          if (!queryElemsTmp) queryElemsTmp = queryElems.slice(0, qlast);
          let unifiedGenerics = [];
          ...
          const passesUnification = unifyFunctionTypes(
            ...
            (mgensScratch) => {
              if (
                fnType.generics.length === 0 &&
                queryElem.generics.length === 0 && fnType.bindings.size === 0 &&
            ...
            const fnTypesRemaining = unifyGenericTypes(
              ...
              (mgensScratch) => {
                ...
                for (const simplifiedMgens of solution.mgens) {
                  ...
                  if (unifiedGenerics !== null) return true;
                }
              },
              unboxingDepth,
              typeNameIds,
      ...
      const unifyFunctionTypeIsMatchCandidate = (
        ...
        if (!typePassesFilter(queryElem.typeFilter, fnType.ty)) return false;
        if (
          fnType.id !== null && fnType.id < 0 && queryElem.id !== null &&
          queryElem.id < 0
        ) {
          ...
          if (fnType.bindings.size < queryElem.bindings.size) return false;
          ...
          if (queryElemPathLength > 0) {
            const fnTypePath = fnType.path !== undefined && fnType.path !== null
              ? fnType.path.split("::")
              : [];
            if (queryElemPathLength > fnTypePath.length) return false;
            let i = 0;
            for (const path of fnTypePath) {
              if (path === queryElem.pathWithoutLast[i]) {
                i += 1;
                if (i >= queryElemPathLength) break;
              }
            }
            if (i < queryElemPathLength) return false;
          }
          return true;
        }
      };
      function unifyFunctionTypeCheckBindings(
        ...
        if (fnType.bindings.size > 0) {
          ...
          for (const [name, constraints] of queryElem.bindings.entries()) {
            if (mgensSolutionSet.length === 0) return false;
            if (!fnType.bindings.has(name)) return false;
      ...
      function unifyFunctionTypeIsUnboxCandidate(
        ...
        if (unboxingDepth >= UNBOXING_LIMIT) return false;
        if (fnType.id !== null && fnType.id < 0) {
      ...
      function containsTypeFromQuery(elems, list, where_clause, typeNameIds) {
        ...
        for (const ty of elems) {
          if (ty.id !== null && ty.id < 0) continue;
      ...
      const checkType = (
        ...
        if (unboxingDepth >= UNBOXING_LIMIT) return false;
        if (
          row.id !== null && elem.id !== null && row.id > 0 && elem.id > 0 &&
          elem.pathWithoutLast.length === 0 && row.generics.length === 0 &&
          elem.generics.length === 0 && row.bindings.size === 0 &&
        ...
        } else {return unifyFunctionTypes(
            [row],
            [elem],
            whereClause,
            mgens,
      ...
      const checkTypeMgensForConflict = (mgens) => {
        if (!mgens) return true;
        const fnTypes = new Set();
        for (const [_qid, fid] of mgens) {
          if (fnTypes.has(fid)) return false;
          fnTypes.add(fid);
        }
        return true;
      };
      function checkPath(contains, path) {
        ...
        pathiter: for (let i = length - clength; i >= 0; i -= 1) {
          ...
          for (let x = 0; x < clength; ++x) {
            ...
            if (parsedQuery.literalSearch && p !== c) continue pathiter;
            else if (
              ...
              if (dist > maxPathEditDistance) continue pathiter;
              dist_total += dist;
            }
          }
      ...
      const innerRunNameQuery = async function* (currentCrate) {
        ...
        let count = 0;
        const prefixResults = [];
        const normalizedUserQuery = parsedQuery.userQuery.replace(/[_"]/g, "")
          .toLowerCase();
        ...
        const flush = async function* (data) {
          ...
          for await (const processed of satr) {
            yield processed;
            count += 1;
            if ((count & 0x7F) === 0) await yieldToBrowser();
        ...
        if (aliasResults) {
          for (const id of aliasResults.matches().entries()) {
            ...
            if (
              name !== null && alias !== null && !idDuplicates.has(id) &&
        ...
        if (elem.normalizedPathLast === "") {
          ...
          for (let id = 0; id < l; ++id) {
            ...
            if (yield* flush(prefixResults)) return;
          }
          return;
        }
        ...
        if (results) {
          ...
          for await (const result of results.substringMatches()) {
            ...
            if (yield* flush(substringResults)) return;
          }
        }
      }.bind(this);
      const innerRunTypeQuery = async function* (
        ...
        const intersectInvertedIndexes = (...idx) => {
          let i = 0;
          const l = idx.length;
          while (i < l - 1 && idx[i] === everything_inverted_index) i += 1;
          const result = [...idx[i]];
          for (; i < l; ++i) {
            if (idx[i] === everything_inverted_index) continue;
            if (idx[i].length < result.length) result.length = idx[i].length;
        ...
        const unpackPostingsList = async (elem, polarity) => {
          if (!elem) return empty_postings_list;
          const typeFilter = itemTypeFromName(elem.typeFilter);
          ...
          const types = (await Promise.all(typePromises)).filter((
            ...
            name !== null && name.toLowerCase() === elem.pathLast && ty &&
          ...
          for (const [id, _name, typeData] of types) {
            if (
              !typeData || typeData[polarity].every((bitmap) => {
                return bitmap.isEmpty();
              })
            ) continue;
            for (
              const { invertedIndex: genericsIdx, queryElem: generics } of upla
            ) {
              for (
                const { invertedIndex: bindingsIdx, queryElem: bindings }
                  of uplb
              ) {
                results.push({
                ...
                if ((results.length & 0x7F) === 0) await yieldToBrowser();
              }
            }
          }
          return results;
        ...
        const unpackPostingsListAll = async (elems, polarity) => {
          ...
          const results = [];
          for (
            const { invertedIndex: firstIdx, queryElem: firstElem }
              of firstPostingsList
          ) {
            for (
              const { invertedIndex: remainingIdx, queryElem: remainingElems }
                of remainingAll
            ) {
              results.push({
        ...
        const unpackPostingsListBindings = async (elems, polarity) => {
          ...
          const [firstKeyIds, firstPostingsList, remainingAll] = await Promise
            .all([
        ...
        const queryPlan = [];
        for (
          const { invertedIndex: inputsIdx, queryElem: inputs } of allInputs
        ) {
          for (
            const { invertedIndex: outputIdx, queryElem: output } of allOutput
          ) {
            ...
            for (const [size, bitmap] of invertedIndex.entries()) {
              checkCounter += 1;
              if ((checkCounter & 0x7F) === 0) await yieldToBrowser();
              if (!queryPlan[size]) queryPlan[size] = [];
        ...
        plan: for (const queryStep of queryPlan) {
            ...
            for (const id of bitmap.entries()) {
              ...
              resultPromises.push(
                this.getFunctionData(id).then(async (fnData) => {
                  if (!fnData || !fnData.functionSignature) return null;
                  checkCounter += 1;
                  if ((checkCounter & 0x7F) === 0) await yieldToBrowser();
                  ...
                  if (
                    ...
                    (isReturnTypeQuery && functionSignature &&
          ...
          for await (
            const result of sortAndTransformResults(
            ...
            if (resultCounter >= MAX_RESULTS) break plan;
            yield result;
            resultCounter += 1;
          }
          resultPromises.length = 0;
        }
        if (resultCounter >= MAX_RESULTS) return;
        for await (
          const result of sortAndTransformResults(
          ...
          if (resultCounter >= MAX_RESULTS) break;
          yield result;
          resultCounter += 1;
        }
      }.bind(this);
  ...
  function printTab(nb) {
    ...
    onEachLazy(document.getElementById("search-tabs").childNodes, (elem) => {
      ...
      } else removeClass(elem, "selected");
      iter += 1;
    });
    ...
    onEachLazy(document.getElementById("results").childNodes, (elem) => {
      ...
      } else removeClass(elem, "active");
      iter += 1;
    });
    if (foundCurrentTab && foundCurrentResultSet) {
      searchState.currentTab = nb;
      const correctionsElem = document.getElementsByClassName(
        "search-corrections",
      );
      if (isTypeSearch) removeClass(correctionsElem[0], "hidden");
      else addClass(correctionsElem[0], "hidden");
    } else if (nb !== 0) printTab(0);
  }
  ...
  function getFilterCrates() {
    ...
    if (elem && elem.value !== "all crates") return elem.value;
    return null;
  }
  function nextTab(direction) {
    const next = (searchState.currentTab + direction + 3) %
  ...
  function focusSearchResult() {
    const target = searchState.focusedByTab[searchState.currentTab] ||
      ...
      document.querySelectorAll("#search-tabs button").item(
...
${obj.displayPath}<span class="${type}">${name}</span>\
      ...
      obj.desc.then((desc) => {
        if (desc !== null) description.insertAdjacentHTML("beforeend", desc);
      });
      descList.push(obj.desc);
      if (obj.displayTypeSignature) {
        const { type, mappedNames, whereClause } = await obj
        ...
        type.forEach((value, index) => {
          ...
          } else displayType.appendChild(document.createTextNode(value));
        });
        if (mappedNames.size > 0 || whereClause.size > 0) {
          ...
          for (const [name, innerType] of whereClause) {
            if (innerType.length <= 1) continue;
            addWhereLineFn();
            const line = document.createElement("div");
            line.className = "where";
      ...
      results.next().then(async (nextResult) => {
        if (nextResult.value) addNextResultToOutput(nextResult.value);
        else {
    ...
    if (firstResult.value) {
      ...
      output.innerHTML += "Example searches:<ul>" + '<li><a href="' +
        getNakedUrl() + '?search=std::vec">std::vec</a></li>' +
        '<li><a href="' + getNakedUrl() +
        '?search=u32+->+bool">u32 -> bool</a></li>' + '<li><a href="' +
        getNakedUrl() + '?search=Option<T>,+(T+->+U)+->+Option<U>">' +
        "Option&lt;T>, (T -> U) -> Option&lt;U></a></li>" + "</ul>";
  ...
  async function showResults(docSearch, results, goToFirst, filterCrates) {
    ...
    if (results.query.error !== null) {
      ...
      const signatureTabTitle = results.query.elems.length === 0
    ...
    if (crateSearch) crateSearch.addEventListener("input", updateCrate);
  ...
  function updateSearchHistory(url) {
    ...
    if (!browserSupportsHistoryApi()) return;
  ...
  async function search(forced) {
    ...
    if (!forced && query.userQuery === currentResults) {
      if (query.userQuery.length > 0) putBackSearch();
      return;
    }
  ...
  function registerSearchEvents() {
    ...
    if (inputElement.value === "") inputElement.value = params.search || "";
    ...
    if (inputElement.form) inputElement.form.onsubmit = onSearchSubmit;
  ...
  const makeUint8ArrayFromBase64 = Uint8Array.fromBase64
    ...
    : ((string) => {
      ...
      for (let i = 0; i < l; ++i) bytes[i] = bytes_as_string.charCodeAt(i);
      return bytes;
    });
  ...
  initSearch(window.Stringdex, window.RoaringBitmap, {
    loadRoot: (callbacks) => {
      for (const key in callbacks) {
        if (Object.hasOwn(callbacks, key)) window[key] = callbacks[key];
      }
      databaseCallbacks = callbacks;
      if (window.searchIndex) window.rr_(window.searchIndex);
    },
    loadTreeByHash: (hashHex) => {
      const script = document.createElement("script");
      script.src = `${ROOT_PATH}search.index/${hashHex}.js`;
      script.onerror = (e) => {
        if (databaseCallbacks) databaseCallbacks.err_rn_(hashHex, e);
      };
    ...
    loadDataByNameAndHash: (name, hashHex) => {
      ...
      script.onerror = (e) => {
        if (databaseCallbacks) databaseCallbacks.err_rd_(hashHex, e);
      };
      document.documentElement.appendChild(script);
    },
  });
} else if (typeof exports !== "undefined") exports.initSearch = initSearch;
