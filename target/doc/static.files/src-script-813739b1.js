...
(function () {
  ...
  window.rustdocToggleSrcSidebar = () => {
    if (document.documentElement.classList.contains("src-sidebar-expanded")) {
      window.rustdocCloseSourceSidebar();
    } else window.rustdocShowSourceSidebar();
  };
  function createSrcSidebar(srcIndexStr) {
    ...
    if (typeof selected_elem !== "undefined") selected_elem.focus();
  }
  function highlightSrcLines() {
    ...
    let to = from;
    if (typeof match[2] !== "undefined") to = parseInt(match[2], 10);
    if (to < from) {
      const tmp = to;
      to = from;
      from = tmp;
  ...
  const handleSrcHighlight = function () {
    ...
    const set_fragment = (name) => {
      ...
      if (browserSupportsHistoryApi()) {
        ...
        highlightSrcLines();
      } else location.replace("#" + name);
      window.scrollTo(x, y);
    };
    return (ev) => {
      let cur_line_id = parseInt(ev.target.id, 10);
      if (isNaN(cur_line_id) || ev.ctrlKey || ev.altKey || ev.metaKey) return;
      ev.preventDefault();
