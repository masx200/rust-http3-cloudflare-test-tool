...
const settingsDataset = (function () {
  ...
  return settingsElement && settingsElement.dataset
    ? settingsElement.dataset
    : null;
})();
function nonnull(x, msg) {
  if (x === null) throw (msg || "unexpected null value!");
  else return x;
}
function nonundef(x, msg) {
  if (x === undefined) throw (msg || "unexpected null value!");
  else return x;
}
...
function onEach(arr, func) {
  for (const elem of arr) if (func(elem)) return true;
  return false;
}
...
function updateLocalStorage(name, value) {
  try {
    if (value === null) window.localStorage.removeItem("rustdoc-" + name);
    else window.localStorage.setItem("rustdoc-" + name, value);
  } catch {}
}
...
function switchTheme(newThemeName, saveTheme) {
  ...
  if (newThemeName === null || themeNames.indexOf(newThemeName) === -1) return;
  if (saveTheme) updateLocalStorage("theme", newThemeName);
...
const updateTheme = (function () {
  ...
  function updateTheme() {
    ...
    } else switchTheme(getSettingValue("theme"), false);
  }
...
if (getSettingValue("use-system-theme") !== "false" && window.matchMedia) {
  if (
    ...
    localStoredTheme !== null && darkThemes.indexOf(localStoredTheme) >= 0
