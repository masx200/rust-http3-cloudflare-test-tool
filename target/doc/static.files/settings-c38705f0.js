  ...
  function changeSetting(settingName, value) {
    ...
    switch (settingName) {
      ...
      case "line-numbers":
        ...
        break;
      case "hide-sidebar":
        if (value === true) addClass(document.documentElement, "hide-sidebar");
        else removeClass(document.documentElement, "hide-sidebar");
        break;
      case "hide-toc":
        if (value === true) addClass(document.documentElement, "hide-toc");
        else removeClass(document.documentElement, "hide-toc");
        break;
      case "hide-modnav":
        if (value === true) addClass(document.documentElement, "hide-modnav");
        else removeClass(document.documentElement, "hide-modnav");
        break;
      case "sans-serif-fonts":
        if (value === true) addClass(document.documentElement, "sans-serif");
        else removeClass(document.documentElement, "sans-serif");
        break;
      case "word-wrap-source-code":
        if (value === true) {
          addClass(document.documentElement, "word-wrap-source-code");
        } else removeClass(document.documentElement, "word-wrap-source-code");
        break;
    }
  }
  ...
  function setEvents(settingsElement) {
    ...
    onEachLazy(
      ...
      (toggle) => {
        ...
        if (settingValue !== null) toggle.checked = settingValue === "true";
        toggle.onchange = () => {
...
<div class="setting-line">\
  ...
  function buildSettingsPage() {
    ...
    const theme_names = (theme_list === null ? "" : theme_list).split(",")
      .filter((t) => t);
    ...
    const innerHTML = `<div class="settings">${
      buildSettingsPageSections(settings)
    }</div>`;
    const el = document.createElement(elementKind);
    el.id = "settings";
    if (!isSettingsPage) el.className = "popover";
    el.innerHTML = innerHTML;
    if (isSettingsPage) {
      const mainElem = document.getElementById(MAIN_ID);
      if (mainElem !== null) mainElem.appendChild(el);
    } else {
  ...
  function displaySettings() {
    ...
    onEachLazy(
      ...
      (el) => {
        ...
        if (checked !== el.checked && val !== null) el.checked = checked;
      },
    );
  }
    ...
    onEachLazy(
      ...
      (settingsButton) => {
        settingsButton.querySelector("a").onclick = (event) => {
          ...
          if (shouldDisplaySettings) displaySettings();
        };
  ...
  setTimeout(() => {
    ...
    if (!isSettingsPage) displaySettings();
    onEachLazy(
      document.querySelectorAll(".settings-menu"),
      (settingsButton) => {
